...
public class Node {
	...
	/* valore di somiglianza in corrispondenza del quale é stato creato il nodo (cluster) */
...
//		Node mergedClusterNode = new Node(value, tagCentroid);
		...
		/* questo nodo é il padre per tutti i cluster fusi */
		Iterator<Node> childrenIterator = this.getChildren().listIterator();
		/* il valore del nodo é calcolato sulla base del valore che 
...
//		this.value = this.value.substring(0, this.value.length() - 1);
		 ...
		 * quando il clustering é completo, si assegnano gli id */
	}
...
//							ancestorA.idNode + " left: " + ancestorA.left + " right: " + ancestorA.right);
	 ...
	 * prosegue per i cluster composti da piú tag) 
	 * 
	 ...
	 * 		// 1 e 2 per esempio) é <= similarity, non lo considero.
	 * 		// Se é maggiore, allora lo prendo e fondo i cluster 1 e 2 nel cluster X
